...
const categories: Category[] = [
  {
    id: 1,
    name: "Engineering",
    queryParam: "Software Engineering",
    count: "1,250 Books",
    icon: "⚙️",
      ...
      <div className="container mx-auto px-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {categories.map((category, index) => (
            <Link
              key={category.id}
              href={`/books?category=${encodeURIComponent(category.queryParam)}`}
            >
              <motion.div
                ...
                className={`relative group p-8 rounded-2xl border border-white/20 bg-gradient-to-br ${category.color} backdrop-blur-md shadow-xl cursor-pointer overflow-hidden h-full`}
              >
